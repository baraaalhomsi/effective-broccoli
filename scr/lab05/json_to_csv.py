...
def csv_to_json(csv_path: str, json_path: str) -> None:
    ...
    if not csv_path.lower().endswith(".csv"):
        raise ValueError(f"file {csv_path} must be .csv")

    if not json_path.lower().endswith(".json"):
        raise ValueError(f"file {json_path} must be .json")
    ...
    # التحقق من أنه ليس فارغ
    if not rows:
        raise ValueError("file CSV is empty")

    header = rows[0]

    # التحقق من صحة كل الصفوف (عدد الأعمدة لازم يطابق الهيدر)
    for i, row in enumerate(rows[1:], start=2):
        if len(row) != len(header):
            raise ValueError(f"invalid CSV format at line {i}")

    # التحويل إلى list of dicts
    data = [dict(zip(header, row)) for row in rows[1:]]

    # إنشاء مجلد الإخراج إذا مش موجود
    ...
    try:
        ...
        raise ValueError(f"wrong while writing JSON: {e}")
