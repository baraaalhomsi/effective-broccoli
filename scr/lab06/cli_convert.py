...
def main():
    parser = argparse.ArgumentParser(
        description="Data conversion tools between different formats"
    )
    subparsers = parser.add_subparsers(
        dest="command", help="Available conversion commands"
    )

    json2csv_parser = subparsers.add_parser("json2csv", help="Convert JSON to CSV")
    json2csv_parser.add_argument(
        "--in", dest="input", required=True, help="Input JSON file"
    )
    json2csv_parser.add_argument(
        "--out", dest="output", required=True, help="Output CSV file"
    )

    csv2json_parser = subparsers.add_parser("csv2json", help="Convert CSV to JSON")
    csv2json_parser.add_argument(
        "--in", dest="input", required=True, help="Input CSV file"
    )
    csv2json_parser.add_argument(
        "--out", dest="output", required=True, help="Output JSON file"
    )
    ...
    csv2xlsx_parser.add_argument(
        "--out", dest="output", required=True, help="Output XLSX file"
    )
